    ...
    <>
      <div className={styles.carouselOuterWrapper}>
        <div className={styles.leftMoveWrraper} onClick={handleLeft}>
          {left !== 0 && (
            <img
              className={`${styles.arrowImg} rotate-180`}
              src="/circle-with-arrow.png"
            />
          )}
        </div>
        <div className={styles.carouselFrame}>
          <div
            className={styles.carouselMovable}
            style={{
              width: `${wide * 100}%`,
              transform: `translateX(-${(100 * left) / wide}%)`,
              gridTemplateColumns: `repeat(${onScreen * wide}, 1fr)`,
            }}
          >
            {children}
          </div>
