    ...
    <div className={styles.detailCardWrapper}>
      ...
      <div className={styles.rightCard}>
        ...
        <div className={styles.ratings}>
          {ratingArr.map(() => (
            <img src="/star.png" className={styles.star} key={Date.now()} />
          ))}
          <span className={styles.ratingText}>{`( ${Math.ceil(
        ...
        >{`Price Rs ${productDetails.discountPercentage}`}</h3>
        <h3
          className={styles.discount}
        >{`Discount ${productDetails.discountPercentage}%`}</h3>
      </div>
    </div>
  );
};
